...
export default function SettingsIntegrations() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      pushToast({
        severity: 'error',
        body: `Failed to generate token: ${err instanceof Error ? err.message : 'Unknown error'}`,
      });
    } finally {
      setGenerating(false);
    }
  ...
  const handleRevoke = async (id: string) => {
    ...
    try {
      ...
      pushToast({
        severity: 'error',
        body: `Failed to revoke token: ${err instanceof Error ? err.message : 'Unknown error'}`,
      });
    } finally {
      setRevoking(null);
    }
  ...
  return (
    <Page width="data">
      <PageHeader
        ...
        title="Integrations"
        description="Manage health sources, synchronization health, and device API tokens."
      />
      <div
        className="settings-integrations-grid"
        style={{
          display: 'grid',
      ...
      >
        ...
        {generatedToken && (
          <GenerateTokenModal token={generatedToken} onClose={() => setGeneratedToken(null)} />
        )}
        ...
        >
          {syncError ? (
            <DataState
              kind="error"
              title="Sync status could not be loaded"
              body="Your connection has not been changed. Retry to refresh its latest status."
              action={
                <Button variant="secondary" onClick={() => void fetchSync()}>
                  Retry
                </Button>
              }
            />
          ...
          >
            ...
            >
              <div>
                ...
                >
                  Once-daily bodyweight push from an iOS Shortcut. Reads your morning weight from
                  Health and posts it to RepOS. No reverse-write.
                </p>
              </div>
              <Button
                variant="secondary"
            ...
            >
              {[
                ...
                {
                  k: 'LAST FIRED',
                  v: syncLoading ? '...' : formatDateTime(sync?.last_success_at ?? null),
                },
          ...
          >
            ...
            >
              <div
                style={{ color: TOKENS.accent, fontSize: 10, letterSpacing: 1, marginBottom: 6 }}
              >
          ...
          >
            ...
            {/* Token table */}
            {tokensLoading ? (
              <DataState kind="loading" title="Loading device tokens" />
            ) : tokensError ? (
              <DataState
                kind="error"
                title="Device tokens could not be loaded"
                body={`Your existing tokens were not changed. ${tokensError}`}
                action={
