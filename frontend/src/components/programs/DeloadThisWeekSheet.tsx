...
export function DeloadThisWeekSheet({
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      const f = Array.from(sheetRef.current.querySelectorAll<HTMLElement>('button:not([disabled])'));
      ...
      pushToast({
        ...
        onAction: () => {
          undoManualDeload(runId)
            ...
            .catch(() => pushToast({ severity: 'error', body: 'Undo failed — the 24h window may have passed.' }));
        },
      });
      onClose(true);
    } catch (e) {
  ...
  return (
    <div
      ...
      ref={sheetRef}
      style={{
        position: 'fixed', inset: 0, background: 'rgba(10,13,18,0.72)',
    ...
    >
      <div style={{
        ...
        borderRadius: '16px 16px 0 0', padding: '24px 24px 32px',
        maxWidth: 560, width: '100%', fontFamily: FONTS.ui,
      }}>
        ...
        <p style={{ fontSize: 14, color: TOKENS.textDim, lineHeight: 1.6, margin: '0 0 18px' }}>
          Reduce remaining-week sets to about half of <Term k="MAV" /> and pin <Term k="RIR" /> to 4.
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          ...
          <button type="button" onClick={() => onClose(false)} disabled={busy} style={cancelBtn}>Cancel</button>
        </div>
      </div>
    </div>
  );
