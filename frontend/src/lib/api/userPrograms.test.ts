...
describe('userPrograms API client', () => {
  ...
  it('lists mine — unwraps { programs: [...] } envelope', async () => {
    (fetch as any).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ programs: [{ id: 'up-1', status: 'draft' }] }),
    });
  ...
  it('listMyPrograms with includePast=true appends ?include=past', async () => {
    (fetch as any).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ programs: [{ id: 'up-2', status: 'abandoned' }] }),
    });
  ...
  it('PATCH applies customizations', async () => {
    (fetch as any).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ id: 'up-1', customizations: { renamed: true } }),
    });
  ...
  it('start materializes', async () => {
    (fetch as any).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ mesocycle_run_id: 'mr-1' }),
    });
    const r = await startUserProgram('up-1', {
  ...
  it('start surfaces template_outdated 409 with must_refork payload', async () => {
    (fetch as any).mockResolvedValueOnce({
      ...
      status: 409,
      text: async () =>
        JSON.stringify({ error: 'template_outdated', latest_version: 3, must_refork: true }),
    });
    await expect(
      startUserProgram('up-1', {
        start_date: '2026-05-05',
        start_tz: 'America/Indiana/Indianapolis',
      }),
    ).rejects.toThrow(/template_outdated|409/);
  });
});
