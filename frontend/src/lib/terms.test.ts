...
describe('W6 term additions', () => {
  it.each(['PAT', 'bearer_token', 'session', 'IANA_timezone', 'truncated_ip_24'] as const)(
    'has TERMS entry for %s',
    (k) => {
...
describe('W2 term additions', () => {
  it.each(['PAR_Q', 'core', 'intro_week', 'soft_gate', 'manual_deload', 'advisory_mode'] as const)(
    'has TERMS entry for %s',
    (k) => {
      expect(TERMS[k as keyof typeof TERMS]).toBeDefined();
      expect(TERMS[k as keyof typeof TERMS].short.length).toBeGreaterThan(0);
