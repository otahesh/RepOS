...
const USER_PROGRAM_WITH_DAY: upApi.UserProgramDetail = {
  ...
  effective_structure: {
    ...
    days: [
      {
        idx: 0, day_offset: 0, kind: 'strength', name: 'Push',
        blocks: [
...
describe('MyProgramPage', () => {
  beforeEach(() => {
    ...
    vi.mocked(exApi.listExercises).mockResolvedValue([
      { id: '1', slug: 'barbell-bench-press', name: 'BB Bench Press', primary_muscle: 'chest', primary_muscle_name: 'Chest', movement_pattern: 'push_horizontal', peak_tension_length: 'mid', skill_complexity: 2, loading_demand: 3, systemic_fatigue: 3, required_equipment: { _v: 1, requires: [] }, muscle_contributions: { chest: 1 } },
      { id: '2', slug: 'dumbbell-bench-press', name: 'DB Bench Press', primary_muscle: 'chest', primary_muscle_name: 'Chest', movement_pattern: 'push_horizontal', peak_tension_length: 'mid', skill_complexity: 2, loading_demand: 3, systemic_fatigue: 3, required_equipment: { _v: 1, requires: [] }, muscle_contributions: { chest: 1 } },
    ] as any)
  ...
  it('on deload choice, opens ConfirmDialog with volume math; on confirm calls startMesocycle({intent:deload})', async () => {
    ...
    const startSpy = vi.spyOn(mesoApi, 'startMesocycle').mockResolvedValue({
      mesocycle_run_id: 'new-run-1', start_date: '2026-05-27', start_tz: 'UTC',
      weeks: 5, status: 'active', current_week: 1, is_deload: true,
    })
    ...
    await waitFor(() => expect(startSpy).toHaveBeenCalledWith({ user_program_id: 'up-1', intent: 'deload' }))
  })
  ...
  it('on run_it_back choice, calls startMesocycle({intent:normal}) and navigates to the new run', async () => {
    vi.spyOn(mesoApi, 'getMesocycle').mockResolvedValue(COMPLETED_RUN)
    const startSpy = vi.spyOn(mesoApi, 'startMesocycle').mockResolvedValue({
      mesocycle_run_id: 'new-run-2', start_date: '2026-05-27', start_tz: 'UTC',
      weeks: 5, status: 'active', current_week: 1, is_deload: false,
    })
    const user = userEvent.setup()
    renderPage()
    await user.click(await screen.findByText(/Run it back/i))
    await waitFor(() => expect(startSpy).toHaveBeenCalledWith({ user_program_id: 'up-1', intent: 'normal' }))
  })
  ...
  it('applying an every-occurrence swap calls patchUserProgram with swap_exercise_all', async () => {
    ...
    await waitFor(() => expect(patchSpy).toHaveBeenCalledWith('up-1', {
      op: 'swap_exercise_all', from_slug: 'barbell-bench-press', to_exercise_slug: 'dumbbell-bench-press',
    }))
  })
