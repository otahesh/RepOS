...
| Q15 | **Dedicated Cloudflare API token holding account-scoped `Access: Apps and Policies Edit`, carrying a 180-day `notAfter` expiry.** Recorded in `docs/runbooks/secret-rotation.md`. Never granted any Access *Revoke* permission (Q17a). Revisit if Cloudflare ever exposes a per-policy **token** scope. | **Amended 2026-08-02 (review of `fc1c2c5`), then corrected again in the review of `88ed01f`.** The original — "attempt the beta resource-scoped 'Access policy admin' role limited to this one policy" — was not implementable: it conflated two Cloudflare systems. "Access Policy Admin" is a resource-scoped **member role** for account members, not a permission offered when minting a token; the `Access: Apps and Policies` group is account-scoped (`com.cloudflare.api.account`) with **no** single-policy variant ([member roles](https://developers.cloudflare.com/fundamentals/manage-members/roles/), [token permissions](https://developers.cloudflare.com/fundamentals/api/reference/permissions/)). The dashboard labels it **Edit**; endpoint docs sometimes say *Write*. **The blast radius is ACCEPTED, and is account-wide.** The first amendment claimed RepOS's fail-closed guards (Q10/Q19/Q22/Q38) contained it — **that was wrong.** Those guards constrain *this application's* policy client; a stolen token is presented to Cloudflare's API directly, with RepOS nowhere in the path, so it inherits the token's full account scope including edit over `ha.jpmtech.com` and `jellyseerr.jpmtech.com`. Nothing RepOS can implement narrows that. The only real containment Cloudflare offers is **expiry**, so the token carries a 180-day `notAfter` ([token restrictions](https://developers.cloudflare.com/fundamentals/api/how-to/restrict-tokens/)) — an enforced bound rather than a procedural one. IP restriction is available too and would narrow it further; not adopted because the Unraid host's address is residential and dynamic. |
...
| Q23 | **Audit rows carry both actor and target, and the actor has two shapes.** `account_events.user_id` is always the **target** (so the event lands in that user's `AccountEventsTimeline`). The actor is recorded in `meta` under a discriminated shape: **human** → `actor_kind='user'`, `actor_user_id`, `actor_email`; **system** → `actor_kind='system'`, `actor_name` (e.g. `'cf_reconciliation'`), and `source` — either `'cutover'` or `'restore'`. Every lifecycle event carries exactly one of the two shapes. | Review clarification. `invited_by` covers invitation only; suspension, role change, and deletion need durable attribution too. This requires **no migration** — `kind` has no CHECK constraint by design (per C-ACCOUNT-EVENTS-ENUM, new kinds extend the TypeScript union) and `meta` is intentionally permissive. Deletion attribution survives via the existing `user_id_at_event` snapshot + `ON DELETE SET NULL`. **Round 7** added the system shape: Q31 had invented `meta.actor='system:cutover'`, a third format matching neither this contract nor its test, and one that lies outright when reconciliation runs during a restore rather than the cutover. Discriminating on `actor_kind` and carrying `source` separately makes the run's origin accurate in both cases. Still no migration — the new fields live in the same permissive `meta`. |
...
**Data step — the authoritative statement is Q35; this section does not restate it.** In summary: 080 carries a data step that guarantees an `active` admin exists, by promotion or by insert, resolved in the order Q35 fixes. It executes **once**, when 080 is applied — `_migrations` is filename-tracked (`migrate.ts:32`), so this is not a continuously-enforced invariant but a one-shot conditional evaluated against the database state at apply time. As of 2026-07-26 the `users` table held exactly one row, `jason.meyer1@gmail.com` (`c5a79f4b-1701-483f-861c-0386b7dabca4`), which was then the founding-admin email constant, so clause (2) was expected to apply. **Amended 2026-08-09, deployment day:** that same row `c5a79f4b` was renamed to `jason@jpmtech.com` before 080 ran, the constant still named the old address, so clause (2) matched nothing and **clause (3) inserted a second admin** — leaving the real account a `member`. Both copies of the constant now read `jason@jpmtech.com` and a test reads the `.sql` text to keep them equal. The general lesson is a property of clause (3), not of this incident: it fires whenever the founding address is absent, and "absent" includes *renamed*.

Lifecycle transitions append `account_events` rows, reusing W6's existing cross-wave contract rather than adding a second audit table. New `kind` values: `user_invited`, `user_activated`, `user_suspended`, `user_reinstated`, `role_changed`, **`user_delete_requested`**, **`user_deleted`**, **`user_imported`** (Q31b — the only kind written with the Q23 system-actor shape). Each is written in the same transaction as the mutation it describes (Q27); `user_deleted` is written immediately before the cascade so its `user_id_at_event` + `user_email_at_event` snapshot survives the FK `SET NULL`. The two deletion events are separate because an interrupted delete may be resumed by a different admin — the pair preserves both the requester and the completer.
...
| Total admin lockout | Break-glass runbook — `docs/runbooks/admin-break-glass.md`, which is authoritative for the exact SQL. Do not inline a shortened form here: the promotion **must** also set `cf_synced_at=NULL` (Q24), and an abbreviated copy that omits it is how the unsafe version got written in the first place. |
...
- **Import contract (Q31b):** an imported row carries `invited_at`, `invite_sent_at NULL`, and exactly one `user_imported` event bearing the Q23 system-actor shape, **committed in the same transaction as the row**; **no mail is sent** during cutover
- **Stale stamp cleared (Q31a):** seed a row whose `cf_synced_at` is non-null but whose policy membership contradicts its status, run reconciliation, and assert the stamp is **actively NULLed** — the case that arises after restoring a post-080 backup
...
- **Audit (Q23):** every lifecycle event records the target (`user_id`) and exactly one actor shape — human (`actor_kind='user'` + id + email) or system (`actor_kind='system'` + `actor_name` + `source`). No event carries a partial or mixed actor. Attribution survives deletion via `user_id_at_event`. A reconciliation run during **restore** records `source='restore'`, not `'cutover'`
...
| **Restore (post-080 dump)** | stale `cf_synced_at` restored | reconciliation actively NULLs mismatched stamps (Q31a) | ✓ | ✓ | ✓ |
