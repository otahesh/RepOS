...
| Q15 | **Dedicated Cloudflare API token holding account-scoped `Access: Apps and Policies Write`.** Recorded in `docs/runbooks/secret-rotation.md` with a rotation cadence. Never granted any Access *Revoke* permission (Q17a). Revisit if Cloudflare ever exposes a per-policy **token** scope. | **Amended 2026-08-02 (review of `fc1c2c5`); supersedes the original "attempt the beta resource-scoped 'Access policy admin' role limited to this one policy".** That instruction was not implementable: it conflated two different Cloudflare systems. "Cloudflare Access Policy Admin" is a **resource-scoped member role** assigned to account *members*, not a permission available when minting an API token — verified against Cloudflare's [member-roles](https://developers.cloudflare.com/fundamentals/manage-members/roles/) and [API token permissions](https://developers.cloudflare.com/fundamentals/api/reference/permissions/) docs, the latter confirming the `Access: Apps and Policies` group is account-scoped (`com.cloudflare.api.account`) with **no** single-policy variant. So the "fall back" was always the only option. The original risk note still stands and is **accepted, not solved**: the account-scoped permission also grants edit over `ha.jpmtech.com` and `jellyseerr.jpmtech.com`, so a RepOS compromise holding this token is a path into home automation. Mitigation is now the application's own fail-closed guards (Q10/Q19/Q22/Q38) plus the rotation cadence, not token scope. |
