...
[~] W9 — User management + invites                               outside the W0–W8 arc; NOT gate-blocking
─── MILESTONE 1 (Beta cutover authorized) ───
...
| **G2** | Every per-user route has a contamination integration test asserting 404/403 (never 200-with-other-user-data); matrix ≥35 routes; `mkUserPair()` fixture exists. | `[~]` partial (W3 user_injuries matrix at `47b875d`; W6 added 6-route account-ops matrix + full-cascade deletion test; W7 added the admin-feedback IDOR/contamination matrix, +2 routes; W9 added the 6-route user-management matrix in `admin-users-contamination.test.ts` — each route proven to refuse a CF-Access `member` with 403 `not_an_admin` **and** a valid `X-Admin-Key` with 403 `cf_access_required`, plus an assertion that the target row is unmutated) | W8.2 (per-route as routes land) | `api/tests/integration/contamination/` |
...
| **G14** | First cohort capped at 10 users; each has signed PAR-Q-lite; each has documented contact path; each saw first-run Beta disclaimer. | `[~]` partial. **The cap is mechanized for EVERY member** — `COHORT_CAP = 10` over `{active, invited, deleting}`, 409 `cohort_cap_reached` on invite *and* reinstate. **Contact path + Beta disclaimer are mechanized only for members who received a W9 invite**, because they live in the invite email body (`SUPPORT_CONTACT` + fixed disclaimer copy) and **two populations never traverse the mailer**: identities imported from the Cloudflare policy by Q31b reconciliation (`cfReconcile.ts` inserts them `invited` and deliberately sends no mail — they were granted out of band) and any user row predating W9. **PAR-Q-lite signing is not W9's at all.** So cutover must still deliver-or-verify contact path + disclaimer for imported and pre-existing identities, and collect PAR-Q for everyone. | cutover-time (PAR-Q for all; contact/disclaimer for imported + pre-existing) | `api/src/constants/users.ts` + `api/src/services/inviteMailer.ts` + `api/tests/routes/admin-users-invite.test.ts`; `par_q_acknowledgments` rows + comms log in PASSDOWN |
...
| **W9** | `docs/superpowers/plans/2026-07-26-w9-user-management.md` | 080–082 | **In-flight** — user management + invites. Sits **outside the original W0–W8 arc and is NOT gate-blocking** (per user direction 2026-07-26: Beta is an active development period, not a feature freeze). Replaces `users.status` / `users.role` for the two removed env vars, so no user-lifecycle change needs a container recreate. Contributes to G2 (+6 routes), G7 (`/settings/users`) and G14 (cap + contact path + disclaimer mechanized). |
