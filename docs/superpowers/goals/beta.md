...
**Last updated:** 2026-07-10 (**pre-cutover prod-window passes executed.** Prod deployed `e03562e` (sequence-workouts + verify-cache + last_used_at debounce; PRs #45–#49 all live). Closed today with evidence: **G9** (baseline committed, zero 5xx, ND3 not needed, 2 budgets renegotiated in PASSDOWN), **G10** (Sev-1 dry-fire 40s), **G12** (prod smoke + Discord delivery), **G13** (service token + live workflow firing green). Note: development detoured 2026-06→07 through programs/logging/sequence-workouts waves (PRs #25–#46) before returning to the cutover checklist. **Next: the supervised browser block — G3 signed-in flows + G5 on-prod restore rehearsal — then G14/G15 cutover-day paperwork and the cutover itself.**)
---
...
| **G3** | Playwright suite covers (a) unauth→CF Access, (b) signed-in→`/`, (c) sign-out clears, (d) sign-out-everywhere revokes bearers, (e) expired JWT mid-set-log buffers + recovers, (f) bearer mint→use→revoke→401. Run against prod CF Access topology in pre-cutover window. | `[~]` (2026-07-10 against prod: **(f) verified live** — mint→200→revoke 204→401; unauth 302→CF Access verified; Shortcut bearer weight POST → 201 **through the real edge** → chart read-back. Remaining: the signed-in browser flows (b/c/d, golden journey, overreaching toast, restore happy-path) — needs a CF Access session) | browser block | PASSDOWN + `frontend/playwright/` |
...
| **G5** | Manual backup → sidecar JSON; `gunzip\|pg_restore -l` integrity check; `tests/dr/restore-into-ephemeral.sh` green; 4 restore tests pass (happy / crash-mid-restore / sigterm-drain / migration-failure-rollback); DR dry-fire within 7 days of cutover. | `[~]` (2026-07-10: `restore-into-ephemeral.sh` GREEN against that day's prod dump — 28 tables, users intact, `last-run.txt` stamped; **remaining: the on-prod restore rehearsal** (`dr-dry-fire.md` steps 1/4/5 — needs a fresh CF Access browser JWT for the typed-RESTORE flow, and re-mints the iOS Shortcut token afterward)) | browser block | `tests/dr/last-run.txt` + PASSDOWN |
...
**Pre-cutover prod-window passes ran 2026-07-10** (this session; full narrative + timestamps in `docs/PASSDOWN.md`): G9/G10/G12/G13 closed with verified evidence; G5 needs only the on-prod restore rehearsal; G3 needs only the signed-in browser flows. Three real defects were found and shipped along the way: the bearer argon2-per-request throughput ceiling (verify-cache PR #47 + debounce PR #48, 159ms→11ms), the rollback.sh APP_SHA masking trap, and the redeploy.sh missing resource caps. Harness gotchas documented in `tests/perf/README.md` (nginx per-IP limiter bypass; stop-api-first Postgres cold restart).

**Next dispatch: the supervised browser block** (operator + agent, ~30 min): CF Access login once → G3 flows (signed-in lands `/`, golden journey, overreaching toast, sign-out, sign-out-everywhere) → G5 restore rehearsal per `docs/runbooks/dr-dry-fire.md` (fresh backup → typed-RESTORE → maintenance banner → post-clear smoke) → single iOS Shortcut token re-mint at the end (covers both destructive tests). Then G14 (PAR-Q + disclaimer + contact path for the N=1 cohort) and G15 (exit-criteria review + weekly cadence on the calendar) at cutover.

Cleanup owed after the window: delete perf user `perf.g9-20260710@repos.test` (cascades ~9k weight rows + programs + token).
