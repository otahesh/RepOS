...
**Last updated:** 2026-05-31 (**W8 — Beta entry gates (build-now) — complete; PR #22 squash-merged to `main` (`e4765e1`)** via per-workstream subagent-driven TDD + a final 5-lens reviewer matrix with adversarial verification (0 Critical, 0 false positives; 2 Important fixed — k6 burst-threshold + weight-date-collision — plus the audit/UX Minors). All 8 CI jobs green; full tree green — api unit 480, api integration 308/7-skip, frontend validate 400, playwright green. Build-now gates **G1/G2/G6/G7/G8/G11 → `[x]`**; the prod-window gates **G3/G5/G9/G10/G12/G13/G15 → `[~]`** (the k6 run, Sev-1 dry-fire, DR dry-fire, prod e2e/feedback smokes, live smoke firing, and the weekly exit reviews are pre-cutover/Beta-period steps — no staging, `project_beta_no_staging`). **PR #22 squash-merged to `main` (`e4765e1`); W8.5 branch protection applied** (all 8 required checks + `enforce_admins` + linear history + 0-approval PR-gate) **and G1 closed** — deliberate-break PR #23 was merge-blocked (API merge → `405 Required status check "api-unit" is failing`), then closed. **Next: the pre-cutover prod-window passes per `docs/runbooks/beta-cutover-checklist.md`, then execute the cutover.**)
...
| **G5** | Manual backup → sidecar JSON; `gunzip\|pg_restore -l` integrity check; `tests/dr/restore-into-ephemeral.sh` green; 4 restore tests pass (happy / crash-mid-restore / sigterm-drain / migration-failure-rollback); DR dry-fire within 7 days of cutover. | `[~]` (W5 shipped + green in CI: `restore.test.ts` happy-path + `restore-crash-recovery.test.ts` + `restore-migration-failure.test.ts` ×4 + `tests/dr/restore-into-ephemeral.sh` + `integrity-check.test.sh`; **only the DR dry-fire within 7 days of cutover remains — cutover-window**) | W5 ✓ / cutover DR dry-fire | `api/tests/integration/restore*.test.ts` + `tests/dr/` |
