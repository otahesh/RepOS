...
| **G14** | First cohort capped at 10 users; each has signed PAR-Q-lite; each has documented contact path; each saw first-run Beta disclaimer. | `[x]` (2026-07-11: N=1 cohort; PAR-Q row verified (v2, 2026-06-26); contact path = beta-triage.md feedback→Discord; first-run disclaimer BUILT this window (PR #58 — it never existed) + surfaced + acked live 17:51 UTC. **W9 re-mechanized the cap**: admission is no longer the CF Access allow-list — that env var is gone — but `COHORT_CAP = 10` over `{active, invited, deleting}`, returning 409 `cohort_cap_reached` on invite *and* reinstate, which reaches every member however provisioned. Disclaimer and PAR-Q also reach everyone: both are `AppShell` first-run gates keyed on a NULL column, not on the invite path. **Residual for growing the cohort:** the *contact path* is delivered only in the W9 invite email, so identities imported from the CF policy by Q31b reconciliation (`cfReconcile.ts` inserts them `invited` and deliberately sends no mail) and any pre-W9 row never receive it — find them with `invite_sent_at IS NULL` and deliver it out of band) | done | PASSDOWN §G14 + `par_q_acknowledgments` + `users.beta_disclaimer_ack_at` + `api/src/constants/users.ts` |
