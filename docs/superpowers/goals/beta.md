...
| **G14** | First cohort capped at 10 users; each has signed PAR-Q-lite; each has documented contact path; each saw first-run Beta disclaimer. | `[~]` partial — **three of the four are mechanized by W9, not prose**: the cap is `COHORT_CAP = 10` enforced against `{active, invited, deleting}` with a 409 (`cohort_cap_reached`) on invite *and* reinstate; the contact path is `SUPPORT_CONTACT` in every invite body; the Beta disclaimer is fixed invite copy. **PAR-Q-lite signing is NOT W9's** and stays cutover-time. | cutover-time (PAR-Q only) | `api/src/constants/users.ts` + `api/src/services/inviteMailer.ts` + `api/tests/routes/admin-users-invite.test.ts`; `par_q_acknowledgments` rows + comms log in PASSDOWN |
