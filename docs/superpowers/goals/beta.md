...
[~] W4 — Desktop authoring + landmarks editor                    implementation complete in worktree branch; pending merge to main
    ...
    [x] W4 self-review (backend/frontend/clinical/security) — no Critical/Important found; deviations documented (zod-v4 record, ConfirmDialog tier=medium, defensive W2-column migrations, joint_stress_profile keys-are-roots)
...
| **G2** | Every per-user route has a contamination integration test asserting 404/403 (never 200-with-other-user-data); matrix ≥35 routes; `mkUserPair()` fixture exists. | `[~]` partial (W3 added user_injuries CRUD matrix at `47b875d`; W4 branch adds 4 full-matrix files: userLandmarks, userProgramStart, userProgramsEveryOccurrence, muscleJointStress — each 401/400/201(or 200)/404 per [I-CONTAM-MATRIX-COMPLETE]) | W8.2 (per-route as routes land) | `api/tests/integration/contamination/` |
...
| **G7** | Every Beta surface reachable from `/` in ≤3 clicks for a logged-in user; prior-mesocycle recap reachable; no surface requires URL knowledge. | `[~]` partial (W3 surfaces verified at `b562338`: /settings/injuries 2 clicks, mid-session swap 3 clicks; W4 branch adds DesktopSwapSheet 3 clicks, /settings/program-prefs 2 clicks, deload 3 clicks — ConfirmDialog is a confirmation modal, not navigation) | W8.8 (audit), all UI waves | `docs/qa/beta-reachability.md` |
...
| **W4** | `docs/superpowers/plans/2026-05-25-w4-desktop-authoring.md` | 041–043 | **Implementation complete in worktree branch (pending merge)** — desktop authoring + landmarks editor with clinical floors/ceilings; `run-it-back` collapsed into `POST /user-programs/:id/start?intent=`. Claimed 043 defensively for the W2-owned `users.par_q_advisory_active` column (IF NOT EXISTS — reconcile when W2 lands). All gates green: api unit 455, api integration 169 (+7 skip), frontend validate 339. |
