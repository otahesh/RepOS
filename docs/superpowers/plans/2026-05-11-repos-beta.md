...
- [ ] **W1.1 — `set_logs` schema (migration 028).** Add `user_id`, `exercise_id`, `rpe`, `client_request_id UUID NOT NULL`, `created_at`, `updated_at` columns. Backfill `user_id` + `exercise_id` from planned_set chain (mesocycle_run.user_id, planned_set.exercise_id). Idempotency dedupe index: `UNIQUE (planned_set_id, date_trunc('minute', performed_at))`. Per-user + per-exercise compound indices for the W3 stalled-PR and overreaching queries. `updated_at` trigger.
  - **Dedupe contract (per QA):** `POST /api/set-logs` with same `client_request_id` UUID returns `200 {deduped: true, set_log: <existing row>}` — full row envelope so callers don't need a second round-trip to read fields. Same minute, same planned_set_id, different client_request_id also returns 200 deduped:true (same-minute double-tap during a workout is the most common offline-replay case).
- [ ] **W1.2 — `set_logs` CRUD routes with 24h audit window.**
  - **Scope:** all four routes require bearer scope `set_logs:write`; CF Access JWTs (webapp path) pass through. The token registry includes a separate `program:write` scope reserved for higher-level program mutations.
  - **Timestamp bounds:** `performed_at` accepted within `(now − 365d, now + 5min]`. The forward-skew tolerance absorbs client-clock drift; the lower bound prevents holding the 24h audit window open indefinitely via a future-dated post.
...
- [ ] **W1.4 — Apple Health Workouts ingest endpoint.** New table `health_workouts`. New scope `health:workouts:write`. New bearer token mint flow. `POST /api/health/workouts` body: `(started_at, ended_at, modality, distance_m?, duration_sec?, source)`. Dedupe key: `(user_id, started_at, source)` with `ON CONFLICT DO NOTHING` — first ingest wins; client correction goes through PATCH in a later wave. `started_at` accepted within `(now − 365d, now + 5min]` per the same audit-pollution rationale as set_logs. 10-writes-per-day-per-user 409 limit; workouts is more chatty than weight (multiple modalities in a day) so the cap is set higher than weight's 5. Error envelope on cap exceeded: `{error: 'rate_limited'}` (matches the alpha-shipped weight contract so offline-queue retry classifiers can share one string). iOS Shortcut authoring guides at `docs/shortcuts/health-weight-sync.md` and `docs/shortcuts/health-workouts-sync.md` (committed to repo, not Notion).
...
- iOS Shortcut authoring guides at `docs/shortcuts/health-weight-sync.md` and `docs/shortcuts/health-workouts-sync.md` are committed and end-to-end-runnable by a user who has never seen the Shortcut.
