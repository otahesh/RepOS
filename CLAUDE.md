...
**Beta (in-flight):** see `docs/superpowers/plans/2026-05-11-repos-beta.md`. 9 waves, 15 binary acceptance gates. W0 (auth flip + cleanup + JWKS test) covered by `docs/superpowers/plans/2026-05-11-beta-W0-auth-flip.md`. **W9 — user management + invites** — `docs/superpowers/plans/2026-07-26-w9-user-management.md`, design frozen at `docs/superpowers/specs/2026-07-26-user-management-design.md`. It sits outside the original W0–W8 arc and is not gate-blocking.

**User management is DATABASE state, not configuration.** Who may sign in is `users.status`; who is an admin is `users.role`. Both are managed by an admin at **`/settings/users`** — invite, suspend, reinstate, delete, retry sync — and **no user-lifecycle change requires a container recreate**. The env vars that used to do this (`CF_ACCESS_ALLOWED_EMAILS`, `REPOS_ADMIN_EMAILS`) were removed by W9; migration 080 records which column replaced each, and `api/tests/integration/w9-env-sweep.test.ts` fails if anything reads them again. Cloudflare Access is *replication*, not the boundary: removing an address from the policy does not end an issued session, so revocations take effect in the DB first and grants take effect in Cloudflare last. First cohort is capped at 10 (`COHORT_CAP`).
