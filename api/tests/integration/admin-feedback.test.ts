...
describe('admin feedback routes', () => {
  ...
  afterAll(async () => {
    ...
    await db.query(`DELETE FROM feedback WHERE id = ANY($1::bigint[])`, [
      [feedbackId, olderUntriagedId, triagedId],
    ]);
    await db.query(`DELETE FROM users WHERE email IN ('boss@repos.test','peon@repos.test')`);
    await app.close();
  });
  ...
  it('lists untriaged-first, newest-first within untriaged (X-Admin-Key path)', async () => {
    const r = await app.inject({
      ...
      headers: { 'x-admin-key': 'w7-admin-key' },
    });
    expect(r.statusCode).toBe(200);
  ...
  it('marks triaged_at idempotently', async () => {
    const r1 = await app.inject({
      method: 'PATCH',
      url: `/api/admin/feedback/${feedbackId}/triage`,
      headers: { 'x-admin-key': 'w7-admin-key' },
    });
  ...
  it('404s a triage on a non-numeric / missing id', async () => {
    const r = await app.inject({
      method: 'PATCH',
      url: `/api/admin/feedback/99999999/triage`,
      headers: { 'x-admin-key': 'w7-admin-key' },
    });
  ...
  it('404s (not 500) a triage on an out-of-bigint-range numeric id', async () => {
    ...
    const r = await app.inject({
      method: 'PATCH',
      url: `/api/admin/feedback/99999999999999999999999/triage`,
      headers: { 'x-admin-key': 'w7-admin-key' },
    });
    expect(r.statusCode).toBe(404);
  });
