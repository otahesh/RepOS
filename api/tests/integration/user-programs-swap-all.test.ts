...
describe('PATCH /user-programs/:id op=swap_exercise_all', () => {
  it('rewrites every block carrying from_slug across all weeks of the program', async () => {
    ...
    const body = r.json<{ customizations: { swaps_all: { from_slug: string; to_slug: string }[]; swaps: any[] } }>();
    ...
    const benchSwaps = swaps.filter((s) => s.from_slug === 'bb-bench-press' && s.to_slug === 'db-bench-press');
