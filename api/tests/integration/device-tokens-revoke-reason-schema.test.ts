...
describe('device_tokens.revoke_reason schema (migration 061)', () => {
  ...
  it('accepts the six enum values (per I-REVOKE-REASON-ENUM)', async () => {
    ...
    for (const reason of [
      ...
      await expect(
        db.query(`UPDATE device_tokens SET revoked_at=now(), revoke_reason=$1 WHERE id=$2`, [
          reason,
          t.id,
        ]),
  ...
  it('rejects unknown revoke_reason values', async () => {
    ...
    await db.query(`INSERT INTO device_tokens (user_id, token_hash) VALUES ($1, $2)`, [
      u.id,
      `aa:bb-${crypto.randomUUID()}`,
    ]);
    await expect(
      db.query(`UPDATE device_tokens SET revoke_reason='garbage' WHERE user_id=$1`, [u.id]),
    ).rejects.toThrow();
    await db.query('DELETE FROM users WHERE id=$1', [u.id]);
  });
