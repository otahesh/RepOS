...
describe('POST /api/user-programs/:id/start with intent param [C-RUN-IT-BACK-ROUTE + D3 + C-IS-DELOAD]', () => {
  it('intent=normal generates a non-deload mesocycle (is_deload=false on both run + day_workouts)', async () => {
    const r = await app.inject({
      method: 'POST',
      url: `/api/user-programs/${upId}/start?intent=normal`,
    ...
    const {
      rows: [run],
    } = await db.query<{ is_deload: boolean }>(`SELECT is_deload FROM mesocycle_runs WHERE id=$1`, [
      body.mesocycle_run_id,
    ]);
    expect(run.is_deload).toBe(false);
    const { rows: dws } = await db.query<{ is_deload: boolean; week_idx: number }>(
      `SELECT is_deload, week_idx FROM day_workouts WHERE mesocycle_run_id=$1`,
      [body.mesocycle_run_id],
    );
    // intent=normal is an accumulation meso, NOT a whole-deload run.
    expect(dws.every((d) => d.is_deload)).toBe(false);
    // W2.5 periodization: the FINAL week is the canonical deload week
    // (is_deload=true); earlier accumulation weeks are false. (Distinct from
    // intent=deload, where the ENTIRE run is_deload — asserted in the next test.)
    const maxWeek = Math.max(...dws.map((d) => d.week_idx));
    for (const d of dws) expect(d.is_deload).toBe(d.week_idx === maxWeek);
  ...
  it('intent=deload sets BOTH mesocycle_runs.is_deload=true AND day_workouts.is_deload=true [C-IS-DELOAD]', async () => {
    const r = await app.inject({
      method: 'POST',
      url: `/api/user-programs/${upId}/start?intent=deload`,
      headers: { authorization: `Bearer ${token}` },
  ...
  it('rejects request when a different active run exists', async () => {
    ...
    await db.query(
      `UPDATE mesocycle_runs SET status='completed', finished_at=now() WHERE user_id=$1 AND status='active'`,
      [userId],
    );
