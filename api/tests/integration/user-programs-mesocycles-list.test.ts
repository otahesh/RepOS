...
beforeAll(async () => {
  ...
  templateId = tpl.id;
  upId = (await mkUserProgram({ userId, templateId: tpl.id, templateVersion: 1, status: 'active' }))
    .id;
...
describe('GET /api/user-programs/:id/mesocycles [WS6.2 / D6]', () => {
  it('returns this program runs newest-first with recap-entry columns', async () => {
    const r = await app.inject({
      method: 'GET',
      url: `/api/user-programs/${upId}/mesocycles`,
  ...
  it('returns 404 for a program id that does not exist', async () => {
    const r = await app.inject({
      method: 'GET',
      url: `/api/user-programs/00000000-0000-0000-0000-000000000000/mesocycles`,
      headers: { authorization: `Bearer ${token}` },
    });
