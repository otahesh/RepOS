...
describe('W8.2 contamination — weight (bearer health:weight:write)', () => {
  it('POST stamps the token owner; B GET never returns A samples', async () => {
    ...
    // Row is owned by A.
    const { rows: aRows } = await db.query(`SELECT 1 FROM health_weight_samples WHERE user_id=$1`, [
      pair.userA.userId,
    ]);
    ...
    const getB = await app.inject({
      method: 'GET',
      url: '/api/health/weight?range=all',
  ...
  it('POST /backfill by B writes only B rows', async () => {
    ...
    const { rows: bRows } = await db.query(`SELECT 1 FROM health_weight_samples WHERE user_id=$1`, [
      pair.userB.userId,
    ]);
