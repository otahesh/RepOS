...
beforeAll(async () => {
  ...
  templateId = tpl.id;
  userBProgramId = (await mkUserProgram({ userId: userB, templateId: tpl.id, templateVersion: 1 }))
    .id;
  userAProgramId = (await mkUserProgram({ userId: userA, templateId: tpl.id, templateVersion: 1 }))
    .id;
...
describe('POST /api/user-programs/:id/start contamination — G2 [I-CONTAM-MATRIX-COMPLETE]', () => {
  ...
  it('returns 401 with no bearer', async () => {
    const r = await app.inject({
      method: 'POST',
      url: `/api/user-programs/${userAProgramId}/start?intent=normal`,
  ...
  it('returns 400 on intent=garbage', async () => {
    const r = await app.inject({
      method: 'POST',
      url: `/api/user-programs/${userAProgramId}/start?intent=garbage`,
      headers: { authorization: `Bearer ${tokenA}` },
      payload: { start_date: '2026-06-01', start_tz: 'UTC' },
    });
    expect(r.statusCode).toBe(400);
  ...
  it('returns 201 for self with intent=deload', async () => {
    const r = await app.inject({
      method: 'POST',
      url: `/api/user-programs/${userAProgramId}/start?intent=deload`,
      headers: { authorization: `Bearer ${tokenA}` },
  ...
  it('user A targeting user B program returns 404', async () => {
    const r = await app.inject({
      method: 'POST',
      url: `/api/user-programs/${userBProgramId}/start?intent=normal`,
      headers: { authorization: `Bearer ${tokenA}` },
      payload: { start_date: '2026-06-01', start_tz: 'UTC' },
    });
