...
describe('resolveUserProgramStructure', () => {
  ...
  it('4. name_override applied: effective_name reflects override', async () => {
    await db.query(`UPDATE user_programs SET customizations=$1::jsonb WHERE id=$2`, [
      JSON.stringify({ name_override: 'My Program' }),
      userProgramId,
    ]);
    try {
      const result = await resolveUserProgramStructure(userProgramId, userId);
      ...
      await db.query(`UPDATE user_programs SET customizations='{}'::jsonb WHERE id=$1`, [
        userProgramId,
      ]);
    }
  });
  ...
  it('5. swaps applied for week_idx=1: first block exercise replaced', async () => {
        ...
        swaps: [
          {
            ...
            day_idx: 0,
            block_idx: 0,
            from_slug: origFirstSlug,
            to_slug: 'barbell-back-squat',
          },
        ],
      }),
      userProgramId,
    ...
    try {
      const result = await resolveUserProgramStructure(userProgramId, userId);
      expect(result).not.toBeNull();
      expect(result!.effective_structure.days[0].blocks[0].exercise_slug).toBe(
        'barbell-back-squat',
      );
        ...
        swaps: [
          {
            ...
            day_idx: 0,
            block_idx: 0,
            from_slug: 'never-existed-this-name',
            to_slug: 'barbell-back-squat',
          },
        ],
      }),
