...
import { assertNotPlaceholderUserId, PLACEHOLDER_USER_ID } from '../../src/bootstrap-runtime.js';
...
describe('assertNotPlaceholderUserId', () => {
  it('throws when the placeholder UUID is written in production', () => {
    expect(() =>
      assertNotPlaceholderUserId(PLACEHOLDER_USER_ID, {
        NODE_ENV: 'production',
  ...
  it('passes null / undefined without throwing (no write yet)', () => {
    expect(() =>
      assertNotPlaceholderUserId(null, { NODE_ENV: 'production' } as NodeJS.ProcessEnv),
    ).not.toThrow();
    expect(() =>
      assertNotPlaceholderUserId(undefined, { NODE_ENV: 'production' } as NodeJS.ProcessEnv),
    ).not.toThrow();
  });
});
