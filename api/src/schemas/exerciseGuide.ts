...
export const ExerciseGuideSeedSchema = z
  .object({
  ...
  // Within-entry uniqueness: a duplicated cue/dont is always an authoring
  // mistake — catch it at validate() time, where content enters the system.
  .superRefine((guide, ctx) => {
    if (new Set(guide.cues).size !== guide.cues.length) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['cues'], message: 'duplicate cue' });
    }
    if (new Set(guide.donts).size !== guide.donts.length) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['donts'], message: 'duplicate dont' });
    }
  });
