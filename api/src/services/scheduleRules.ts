...
export function validateFrequencyLimits(structure: ProgramTemplateStructure): ScheduleWarning[] {
  ...
  const trainingDays = structure.days.filter(
    (d) => d.kind !== 'cardio' || (d.blocks?.length ?? 0) > 0,
  );
  if (trainingDays.length >= 7) {
    out.push({
      code: 'too_many_days_per_week',
      severity: 'block',
      message: '7 training days/week — recovery debt is unavoidable. Drop a day.',
    });
  } else if (trainingDays.length === 6) {
    out.push({
      code: 'too_many_days_per_week',
      severity: 'warn',
      message: '6 days/week is the cap. Watch sleep and food.',
    });
  }
...
export function validateCardioScheduling(structure: ProgramTemplateStructure): ScheduleWarning[] {
  ...
  const isHeavyLower = (d: ProgramTemplateStructure['days'][number]) =>
    d.blocks.some(
      (b: any) => HEAVY_LOWER_PATTERNS.has(b.movement_pattern) && (b.target_rir ?? 99) <= 2,
    );
  const hasInterval = (d: ProgramTemplateStructure['days'][number]) =>
    d.blocks.some(
      (b: any) => b.cardio && (b.cardio.target_zone === 4 || b.cardio.target_zone === 5),
    );
  ...
  for (const d of structure.days) {
    if (isHeavyLower(d) && hasInterval(d)) {
      out.push({
        code: 'cardio_interval_too_close',
        severity: 'warn',
        day_idx: d.idx,
        message:
          'Z4/Z5 cardio same day as heavy lower — interference is high. Move to a different day or downgrade to Z2 ≤ 30 min.',
      });
    }
  }
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    if (hasInterval(prev) && isHeavyLower(curr)) {
      out.push({
        code: 'hiit_day_before_heavy_lower',
        severity: 'warn',
        day_idx: curr.idx,
        message:
          'HIIT the day before heavy lower — quad fatigue carries. Swap day order or move HIIT 48h away.',
      });
    }
  }
  return out;
